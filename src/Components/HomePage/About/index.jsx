export default function About() {
  return (
    <>
      {/* about-area-start */}
      <section className="about-area pt-95 pb-60" id="about">
        <div className="container">
          <div className="row">
            <div className="col-lg-5">
              <div className="about-5">
                <div className="about-5-section mb-70">
                  <div className="tpbanner__sub-title mb-15">
                    <span>A PROPOS</span>
                    <i>
                      <svg
                        width={150}
                        height={40}
                        viewBox="0 0 150 40"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                      >
                        <rect
                          width={150}
                          height={40}
                          fill="url(#pattern3)"
                          fillOpacity="0.1"
                        />
                        <defs>
                          <pattern
                            id="pattern3"
                            patternContentUnits="objectBoundingBox"
                            width={1}
                            height={1}
                          >
                            <use
                              xlinkHref="#image0_859_3410"
                              transform="translate(-0.0584971) scale(0.00611611 0.0203396)"
                            />
                          </pattern>
                          <image
                            id="image0_859_3410"
                            width={180}
                            height={50}
                            xlinkHref="data:image/png;base64,"
                          />
                        </defs>
                      </svg>
                    </i>
                  </div>

                  <h2 className="tpsection__title mb-15">Notre raison d'être</h2>
                  <p>
                    Chez Rézolusoft, nous aspirons à être le leader en matière
                    de solutions numériques sur mesure, en offrant des outils et
                    des services qui non seulement résolvent les problèmes
                    actuels, mais aussi anticipent les besoins futurs. En
                    travaillant en étroite collaboration avec nos clients, nous
                    visons à créer un impact positif et durable, en simplifiant
                    la vie et en propulsant les entreprises vers le succès.
                  </p>
                </div>
              </div>
            </div>
            <div className="col-lg-2"></div>
            <div className="col-lg-5">
              <div className="about-5-thumb p-relative">
                <img
                  className="tpchoose-border-anim"
                  src="assets/img/bg/about-5-bg-2.jpg"
                  alt=""
                />
                <a
                  href="https://www.youtube.com/watch?v=TYYf8zYjP5k"
                  className="popup-video"
                >
                  <div className="about-5-shape">
                    <div className="about-5-shape-one">
                      <img
                        width="40px"
                        src="assets/img/shape/video-blue.png"
                        alt=""
                      />
                    </div>
                    <div className="about-5-shape-two">
                      <img
                        width="45px"
                        src="assets/img/shape/video-white.png"
                        alt=""
                      />
                    </div>
                    <div className="about-5-video-icon">
                      <i className="fa-solid fa-play" />
                    </div>
                  </div>
                </a>
              </div>
            </div>
          </div>
        </div>
      </section>
      {/* about-area-end */}
    </>
  );
}
