export default function Care() {
  return (
    <>
      <>
        {/* funfact-area-start */}
        <section className="funfact-area pb-60">
          <div className="container">
            <div className="tpfunfact p-relative">
              <div
                className="tpfunfact-bg theme-bg-2"
                data-background="assets/img/shape/funfact-1.png"
              >
                <div className="row">
                  <div className="col-lg-12">
                    <div className="tpsection__content feature-white-section text-center">
                      <div className="tpbanner__sub-title mb-15">
                        <span>UBUNTU</span>
                        <i>
                          <svg
                            width={150}
                            height={36}
                            viewBox="0 0 150 36"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            xmlnsXlink="http://www.w3.org/1999/xlink"
                          >
                            <rect
                              x="0.00012207"
                              width={150}
                              height={36}
                              fill="url(#pattern5)"
                              fillOpacity="0.1"
                            />
                            <defs>
                              <pattern
                                id="pattern5"
                                patternContentUnits="objectBoundingBox"
                                width={1}
                                height={1}
                              >
                                <use
                                  xlinkHref="#image0_853_2637"
                                  transform="translate(-0.0507936) scale(0.00603175 0.0205405)"
                                />
                              </pattern>
                              <image
                                id="image0_853_2637"
                                width={180}
                                height={50}
                                xlinkHref="data:image/png;base64,"
                              />
                            </defs>
                          </svg>
                        </i>
                      </div>
                      <h2 className="tpsection-title tpsection-title-white mb-15">
                        Notre Engagement Solidaire
                      </h2>
                      
                    </div>
                  </div>
                </div>
                <div className="tpfunfact-shape d-none d-md-block">
                  <div className="tpfunfact-shape-one">
                    <img src="assets/img/shape/funfact-2.png" alt="" />
                  </div>
                  <div className="tpfunfact-shape-two">
                    <img src="assets/img/shape/funfact-3.png" alt="" />
                  </div>
                </div>
              </div>
              <div className="tpfunfact-box">
                <div className="row justify-content-center">
                  <div className="col-lg-7">
                    <div className="tpfunfact-wrapper text-center mb-50 p-3">
                    
                      <div className="tpfunfact-count">
                        <a href="https://olanike.org" target="blank">
                        <img src="assets/img/shape/icons8-care-100.png" width={80} alt="" />

                        </a>
                        <p>
                          Chez Rézolusoft, nous croyons fermement en
                          l'importance de redonner à la communauté. C'est
                          pourquoi nous avons pris l'engagement de reverser 5%
                          de nos prestations à une association caritative. En
                          choisissant de travailler avec nous, vous contribuez
                          non seulement à la croissance de votre entreprise,
                          mais vous participez également à une cause plus
                          grande, soutenant ceux qui en ont le plus besoin.
                          Ensemble, nous pouvons faire une différence.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
        {/* funfact-area-end */}
      </>
    </>
  );
}
